...
$valido=true;
 $consulta2="SELECT * FROM usuario where usuario='$nickname' AND contrasena='$contrasena'";
         $result=sqlsrv_query($consulta2) or die (mysql_error());
...
$listado = sqlsrv_query($query_limit_listado, $tienda) or die(sqlsrv_error());
$row_listado = sqlsrv_fetch_assoc($listado);

if (isset($_GET['totalRows_listado'])) {
  $totalRows_listado = $_GET['totalRows_listado'];
} else {
  $all_listado = sqlsrv_query($query_listado);
...
<div id="toppanel">
	<div id="panel">
		<div class="content clearfix">
			...
			<div class="left">
				<!-- Login Form -->
				<form class="clearfix" action="cat_procesadores.php" method="post">
					<h1>Miembros</h1>
...
<div id="total">
	...
	if ($contador==0){
      ...
      <?php } while ($row_listado = sqlsrv_fetch_assoc($listado)); ?>
      </table></div>
      <div id="paginacion">
